...
import { ArrowLeft, Plus, Clock, AlertCircle, CheckCircle2, Pencil, Trash2, X, Save, Calendar, User, Bell, Paperclip, Download } from 'lucide-react';
...
const ProjectDetails = () => {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', priority: 'MEDIUM', status: 'TODO', dueDate: '', assigneeId: '', reminder: false });
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch with query params for filtering/searching
      const queryParams = new URLSearchParams();
      if (searchQuery) queryParams.append('search', searchQuery);
      if (filterPriority !== 'ALL') queryParams.append('priority', filterPriority);

      const taskRes = await api.get(`/tasks/project/${id}?${queryParams.toString()}`);
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      ...
      setNewTask({ title: '', description: '', priority: 'MEDIUM', status: 'TODO', dueDate: '', assigneeId: '', reminder: false });
  ...
  const handleFileUpload = async (e, taskId) => {
    ...
    try {
      const res = await api.post(`/tasks/${taskId}/attachment`, formData, {
  ...
  return (
    <div className="project-details">
      <div className="details-header">
        ...
        <div className="filters-bar" style={{ display: 'flex', gap: '16px', marginTop: '20px', alignItems: 'center' }}>
          <input 
            type="text" 
        ...
        <div className="modal-overlay animate-fade-in" onClick={() => setViewingTask(null)}>
          <div className="modal-content glass-panel" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <div className="view-modal-section" style={{ marginTop: '24px' }}>
                <h4 style={{ fontSize: '0.85rem', textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: '8px', letterSpacing: '0.5px' }}>Attachments</h4>
                {viewingTask.attachmentUrl ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <Paperclip size={18} color="var(--primary-color)" />
                    <a href={viewingTask.attachmentUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary-color)', textDecoration: 'none', fontWeight: '500', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {viewingTask.attachmentName || 'Attachment'}
                    </a>
                    <a href={viewingTask.attachmentUrl} target="_blank" rel="noopener noreferrer" style={{ padding: '6px', backgroundColor: 'var(--bg-primary)', borderRadius: '4px', color: 'var(--text-secondary)' }}>
                      <Download size={16} />
                    </a>
                  </div>
                ) : (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <label style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', backgroundColor: 'var(--bg-secondary)', borderRadius: '6px', border: '1px dashed var(--border-color)', color: 'var(--text-secondary)', fontSize: '0.9rem', transition: 'all 0.2s' }}>
                      <Paperclip size={16} />
                      Attach a File
                      <input type="file" style={{ display: 'none' }} onChange={(e) => handleFileUpload(e, viewingTask.id)} />
                    </label>
                  </div>
                )}
              </div>
  ...
  return (
    <div className="task-card glass-panel" onClick={onClick} style={{ cursor: 'pointer' }}>
      ...
      <div className="task-footer" onClick={e => e.stopPropagation()}>
        ...
        {task.reminder && (
          <div className="task-reminder-tiny" title="Reminder Set" style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--warning-text)', fontSize: '0.75rem', fontWeight: '500' }}>
            <Bell size={12} />
          </div>
        )}

        {task.attachmentUrl && (
          <div className="task-reminder-tiny" title="Has Attachment" style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--primary-color)', fontSize: '0.75rem', fontWeight: '500' }}>
            <Paperclip size={12} />
          </div>
        )}
