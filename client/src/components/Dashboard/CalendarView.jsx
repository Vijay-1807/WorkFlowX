...
import { ChevronLeft, ChevronRight, AlertCircle, Clock, CheckCircle2, Bell } from 'lucide-react';
                      ...
                      {hasTasks && (
                        <div className="cal-tasks-container">
                          {dayTasks.slice(0, 3).map(t => (
                            <div key={t.id} className={`cal-task-chip priority-${t.priority.toLowerCase()}`} title={t.title}>
                              <span className="cal-task-chip-dot"></span>
                              <span className="cal-task-chip-text">{t.title}</span>
                            </div>
                          ))}
                          {dayTasks.length > 3 && (
                  ...
                  {selectedTasks.map(task => (
                    <div key={task.id} className="side-task-card">
                      <div className="side-task-header">
                        ...
                        {task.reminder && <Bell size={14} style={{ color: 'var(--warning-text)', marginLeft: '6px' }} title="Reminder Set" />}
                      </div>
